...
This module provides the prompt-builder. Dispatch + worker logic
will live in `routes/concept.py` + `worker/processor.py` once the
backend route ships in step 3 of this phase.
"""
...
class AnglePrompt(TypedDict):
    ...
    angle: str  # e.g. "front", "back", "left profile", "3/4 front-right"
    prompt: str
...
def build_multiview_angle_prompt(angle: str, is_root: bool) -> str:
    ...
    For the ROOT angle (typically "front"), the prompt establishes the
    canonical view and gets dispatched as a fresh `gen_image` from the
    upstream Concept node's reference. For SUBSEQUENT angles, the
    prompt is a rotation directive applied via `edit_image` against
    the root angle's output.

    The system prompt differs subtly between root and child:
      - Root: "establish the canonical FRONT view"
      - Child: "rotate the base image to <angle>; preserve identity"
    """
    if is_root:
        return (
            _MULTIVIEW_CORE
            + "\n\nMODE: ROOT angle. This is the reference view all "
            "other angles will be edited from. Establish a clean, "
            "centred composition with maximum design clarity."
        )
    return (
        _MULTIVIEW_CORE
...
def angles_for_preset(preset_key: str) -> list[str]:
    ...
    Mirrors `frontend/src/constants/concept.ts > MULTIVIEW_PRESETS`.
